...
class ResistanceEngine:
    ...
    async def run(self) -> np.ndarray:
        ...
        for row in range(n_rows):
            for col in range(n_cols):
                ...
                resistance_grid[row, col] = self._calculate_cell_resistance(lat, lng)

        # Store as project metadata (in memory for now, used by connectivity engine)
    ...
    def _calculate_cell_resistance(self, lat: float, lng: float) -> float:
        """Calculate resistance for a single cell based on simulated landscape features."""
        # Base: forest coverage (lower = less resistant)
        forest_cover = 0.5 + 0.3 * np.sin(lat * 8) * np.cos(lng * 6) + 0.2 * np.sin((lat + lng) * 12)
        forest_cover = max(0, min(1, forest_cover))

        base_resistance = 10 + (1 - forest_cover) * 60  # Range: 10-70
        ...
        # Water bodies add moderate resistance
